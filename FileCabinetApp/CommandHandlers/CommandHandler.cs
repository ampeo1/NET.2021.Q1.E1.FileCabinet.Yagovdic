...
namespace FileCabinetApp.CommandHandlers
{
    public class CommandHandler : CommandHandlerBase
    {
        ...
        public override void Handle(AppCommandRequest command)
        {
            ...
            var index = Array.FindIndex(this.commands, 0, this.commands.Length, i => i.Item1.Equals(command.Command, StringComparison.InvariantCultureIgnoreCase));
            if (index >= 0)
            {
                this.commands[index].Item2(command.Parameters);
            }
            else
            {
                PrintMissedCommandInfo(command.Command);
                throw new ArgumentException("Command not found", $"{command.Command}");
            }
        }
        ...
        private static void Edit(string parameters)
        {
            ...
            try
            {
                ...
                Program.fileCabinetService.FindIndexById(id);
            }
            ...
            try
            {
                Program.fileCabinetService.EditRecord(dataRecord);
        ...
        {
            return Program.fileCabinetService.FindByFirstName(firstname);
        }
        ...
        {
            return Program.fileCabinetService.FindByLastname(lastname);
        }
        ...
        private static IReadOnlyCollection<FileCabinetRecord> FindByBirthDay(string birthday)
        {
            ...
            return Program.fileCabinetService.FindByBirthDay(date);
        }
